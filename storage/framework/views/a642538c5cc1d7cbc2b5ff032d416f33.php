    ...
    async function initiatePayment() {
        ...
        try {
            ...
            const payload = {
                ...
                email: email,
                contact: contact,
                psychometric_test: document.querySelector('input[name="psychometric_test"]:checked')?.value // Optionnel
            };

            const response = await fetch('/initiate-payment', {
                method: 'POST',
                body: JSON.stringify(payload), // Envoi du payload
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json', // Format JSON
                    'Accept': 'application/json' // Accepter JSON
                }
            });

            // Vérification et parsing de la réponse JSON
            const data = await response.json();
            if (data.success) {
                console.log('URL de paiement générée :', data.payment_url);
                window.location.href = data.payment_url; // Redirection vers l'URL de paiement
            } else {
                alert('Erreur lors de l\'initiation du paiement : ' + (data.message || 'Veuillez réessayer.'));
            }
        } catch (error) {
