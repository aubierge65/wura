...
<body>
    <div class="ll-invoice-wrapper" id="download-section">
        ...
        <div class="ll-invoice-download">
            <form action="{{ Auth::check() ? (auth()->user()->role === 'admin' ? route('admin.transaction.invoice.download', $transaction->id) : (auth()->user()->role === 'company' ? route('company.transaction.invoice.download', $transaction->id) : '')) : '' }}" method="POST">
                @csrf
                <button type="submit">
