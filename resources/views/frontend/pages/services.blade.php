        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="extraOptionsForm">
                    ...
                    <div class="mb-3">
                        <h5 for="deliveryTime" class="form-label">Choisir le délai de livraison</h5>
                        <select class="form-select" id="deliveryTime">
                            <option value="24h">Délai 24h</option>
                            <option value="72h">Délai 72h</option>
                            <option value="7days">Délai 7 jours</option>
                        </select>
                    </div>
                </form>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="saveOptions()">Enregistrer</button>
            </div>
        </div>
    </div>
</div>
...
@section('script')
<script>
    // Définir les montants pour chaque niveau et chaque délai
    const amounts = {
        "beginner": {
            "24h": {
                "standard": 10000,
    ...
    function calculateTotal() {
        ...
        if (document.getElementById("englishTranslation").checked) {
            totalAmount += baseAmount * 0.75; // Traduction anglaise
        }
        if (document.getElementById("modifiableVersion").checked) {
            totalAmount += baseAmount * 0.50; // Version modifiable
        }
        if (document.getElementById("motivationLetter").checked) {
            totalAmount += 5000; // Lettre de motivation
        }
        if (document.getElementById("designCustomization").checked) {
            totalAmount += baseAmount * 0.50; // Personnalisation du design
        }
    ...
    function saveOptions() {
        ...
        // Ici tu peux ajouter la logique pour enregistrer les options sélectionnées, si nécessaire
    }
</script>
@endsection