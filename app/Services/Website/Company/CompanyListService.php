...
class CompanyListService
{
    ...
    //     if ($request->has('letter') && $request->letter != null) {

    //         // session(['header_search_role' => 'company']);

    //         $letter = $request->letter;
    //         $query->whereHas('user', function ($q) use ($letter) {
    ...
    //         'letter' => ($request->has('letter') ? $letter : '')
    //     ];
    // }
    ...
    public function execute($request): array
    {
        ...
        if ($request->has('industry_type') && $request->industry_type !== null) {
            $industry_type = IndustryTypeTranslation::whereIn('name', $request->industry_type)->where('locale', "fr")->pluck('industry_type_id');
            $query->whereIn('industry_type_id', $industry_type);
        }
        ...
        if ($request->has('organization_type') && $request->organization_type !== null) {
            ...
            $organization_type = OrganizationTypeTranslation::whereIn('name', $request->organization_type)->where('locale', "fr")->pluck('organization_type_id');

            $query->whereIn('organization_type_id', $organization_type);
        }

        // Team Size
        if ($request->has('team_size') && $request->team_size !== null) {
            
            $team_size = TeamSizeTranslation::whereIn('name', $request->team_size)->where('locale', "fr")->pluck('team_size_id');
            $query->whereIn('team_size_id', $team_size);
        }

      // $companies = $query->latest('activejobs')->paginate(12);
        $companies = $query->latest('created_at')->paginate(12);
