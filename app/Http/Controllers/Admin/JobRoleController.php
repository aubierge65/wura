...
class JobRoleController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $jobRoles = JobRole::withCount('jobs')->with('category')->paginate(20);
            $app_language = Language::latest()->get(['code', 'name']);
            $job_categories = JobCategory::all()->sortBy('name');

            return view('backend.JobRole.index', compact('jobRoles', 'app_language', 'job_categories'));
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            $job_role->category_id = $request->category_id;
            $job_role->save();
    ...
    public function edit(JobRole $jobRole)
    {
        try {
            ...
            $app_language = Language::latest()->get(['code', 'name']);
            $job_categories = JobCategory::all()->sortBy('name');
            return view('backend.JobRole.index', compact('jobRole', 'jobRoles', 'app_language', 'job_categories'));
        } catch (\Exception $e) {
            flashError('An error occurred: '.$e->getMessage());
    ...
    public function update(Request $request, JobRole $jobRole)
    {
        try {
            ...
            $jobRole->category_id = $request->category_id;
